...
def create_payload(excel_file_path, ref_dir):
    ...
    step2 = {
        ...
        # Corrigido: Mapeando colunas da aba "Macro Dados" para os campos do JSON
    ...
    # Mapeamento manual dos nomes da aba 'Receitas' (colunas)
    # para os nomes do CSV 'frd_categoria_servico' (valores)
    mapa_servicos_nomes = {
    ...
    # Mapeia os nomes das colunas do Excel para os IDs de serviço
    ...
    if df_receitas.empty:
        ...
        for col_nome, fk_cs_id in mapa_servico_final.items():
            ...
            if col_nome in receitas_data:
                ...
                if valor_receita > 0:
                    item_servico = {
                        "fk_cs_id": int(fk_cs_id),
                        "ps_periodo_repasse": None,
                        "ps_repasse_aquisicao_equipamentos": None,
                        "ps_valor": valor_receita # Usando o valor da aba
                    }
    ...
    df_cat_despesa['categoria_clean'] = df_cat_despesa['cde_categoria'].str.strip().str.lower()
    mapa_cat_despesa = df_cat_despesa.set_index('categoria_clean')['cde_id'].to_dict()
    ...
    for _, row in df_despesas.iterrows():
        ...
        # Pega o valor bruto da célula 'Categoria'
        categoria_cell_value = row.get('Categoria')
        
        # Só atualiza a categoria atual se a célula NÃO for Nula/NaN
        if pd.notna(categoria_cell_value):
            nome_categoria = str(categoria_cell_value).strip().lower()
            categoria_atual = nome_categoria
            fk_cd_id_atual = mapa_cat_despesa.get(categoria_atual)
            if not fk_cd_id_atual:
                 print(f"  [AVISO] Categoria de despesa não mapeada: '{row.get('Categoria')}'", file=sys.stderr)
        
        nome_despesa = str(row.get('Nome', '')).strip().lower()
        valor_despesa = safe_float(row.get('Valor'))
        
        if nome_despesa and valor_despesa > 0 and fk_cd_id_atual:
